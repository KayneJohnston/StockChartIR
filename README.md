...
| [`docs/15_starting_valuation.md`](docs/15_starting_valuation.md) | Conditioning the result on how expensive the market was when a lifetime began, using only the dividend yield an investor could actually observe -- then reading the same yield again at the retirement date, where it turns out to matter more and in the other direction |
